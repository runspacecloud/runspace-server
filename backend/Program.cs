...
app.Use(async (ctx, next) =>
{
    ...
    ctx.Response.Headers["Permissions-Policy"] = "camera=(self), microphone=(self), geolocation=(), payment=(), usb=()";
...
app.MapPost("/api/auth/2fa/verify", async (HttpContext ctx) =>
{
    ...
    Console.WriteLine($"[2FA VERIFY] user={u} utc={DateTime.UtcNow:o}");
...
app.MapPost("/api/chat/upload-image", async (HttpContext ctx) =>
{
    ...
    bool _isAdmin; using (var _adb = DbHelpers.OpenDb()) { using var _ac = _adb.CreateCommand(); _ac.CommandText = "SELECT IsAdmin FROM AuthUsers WHERE Username=$u"; _ac.Parameters.AddWithValue("$u", u); var _ar = _ac.ExecuteScalar(); _isAdmin = _ar != null && Convert.ToInt32(_ar) == 1; } if (!_isAdmin && !limiter.IsAllowed(u, "chat_upload", _uploadLimit, 3600)) return Results.Json(new { error = "Rate limit för uppladdningar." }, statusCode: 429);
...
app.MapPost("/api/sponsors", async (HttpContext ctx, HttpRequest request) =>
{
    ...
    {
        var uploadsDir = "/var/lib/runspace/uploads/sponsors";
...
app.MapDelete("/api/sponsors/{id:int}", (int id, HttpContext ctx) =>
{
    ...
    {
        var fullPath = $"/var/lib/runspace{logoPath}";
...
app.MapGet("/api/spaces", (HttpContext ctx) =>
{
    ...
    while (r.Read()) {
        spaces.Add(new {
            id = r.GetInt64(0), publicId = r.GetString(1), name = r.GetString(2),
            description = r.IsDBNull(3) ? "" : r.GetString(3), iconUrl = r.IsDBNull(4) ? "" : r.GetString(4),
            memberCount = r.GetInt32(5), role = r.GetString(6), ownerId = r.GetInt64(7), ownerName = r.GetString(8)
        });
    }
...
app.MapGet("/api/market/profile/{userId}", (string userId, HttpContext ctx) =>
{
    ...
    {
        scripts.Add(new {
            id = sr.GetInt64(0), name = sr.GetString(1), slug = sr.GetString(2),
            description = sr.GetString(3), category = sr.GetString(4),
            price = sr.GetInt64(5), totalSales = sr.GetInt64(6), isVerified = sr.GetInt64(7) == 1
        });
    }
    sr.Close();
    ...
    {
        feedback.Add(new {
            rating = fr.GetInt64(0), comment = fr.GetString(1), pricePaid = fr.GetInt64(2),
...
public static class AppConfig
{
    ...
    public const string DataDir = "/var/lib/runspace";
    ...
    public const string UploadsDir = "/var/lib/runspace/uploads";
    public const string AvatarUploadDir = "/var/lib/runspace/uploads/avatars";
    public const string ChatUploadDir = "/var/lib/runspace/uploads/chat";
    public const string MarketDir = "/var/lib/runspace/market";
...
public class SecurityAuditLogger { public void LogAuthEvent(string u, string ip, string type, bool ok, string details = "") { try { using var db = DbHelpers.OpenDb(); using var c = db.CreateCommand(); c.CommandText = "INSERT INTO SecurityAuditLog (EventType,Username,Details,Success,Timestamp) VALUES ($t,$u,$d,$s,$ts)"; c.Parameters.AddWithValue("$t", type); c.Parameters.AddWithValue("$u", u ?? ""); c.Parameters.AddWithValue("$ip", ip ?? ""); c.Parameters.AddWithValue("$d", (details ?? "").Length > 500 ? details![..500] : details ?? ""); c.Parameters.AddWithValue("$s", ok ? 1 : 0); c.Parameters.AddWithValue("$ts", DateTime.UtcNow.ToString("o")); c.ExecuteNonQuery(); } catch { } } public void LogSuspiciousActivity(string ip, string type, string details) => LogAuthEvent("", ip, type, false, details); }
...
public class ChatHub : Hub
{
    ...
    // Private Calls
    public async Task CallUser(string toUser) { var me = Context.User?.Identity?.Name?.Trim().ToLowerInvariant(); var target = (toUser ?? "").Trim().ToLowerInvariant(); Console.WriteLine($"[CALL] CallUser from={me} to={target} conn={Context.ConnectionId}"); if (string.IsNullOrWhiteSpace(me)) return; if (string.IsNullOrWhiteSpace(target) || target == me) return; await Clients.User(target).SendAsync("IncomingCall", me); }
    public async Task AcceptCall(string fromUser) { var me = Context.User?.Identity?.Name?.Trim().ToLowerInvariant(); var target = (fromUser ?? "").Trim().ToLowerInvariant(); Console.WriteLine($"[CALL] AcceptCall from={me} to={target} conn={Context.ConnectionId}"); if (string.IsNullOrWhiteSpace(me)) return; await Clients.User(target).SendAsync("CallAccepted", me); }
    public async Task RejectCall(string fromUser) { var me = Context.User?.Identity?.Name?.Trim().ToLowerInvariant(); var target = (fromUser ?? "").Trim().ToLowerInvariant(); Console.WriteLine($"[CALL] RejectCall from={me} to={target} conn={Context.ConnectionId}"); if (string.IsNullOrWhiteSpace(me)) return; await Clients.User(target).SendAsync("CallRejected", me); }
    public async Task EndCall(string otherUser) { var me = Context.User?.Identity?.Name?.Trim().ToLowerInvariant(); var target = (otherUser ?? "").Trim().ToLowerInvariant(); Console.WriteLine($"[CALL] EndCall from={me} to={target} conn={Context.ConnectionId}"); if (string.IsNullOrWhiteSpace(me)) return; await Clients.User(target).SendAsync("CallEnded", me); }
    public async Task SendCallOffer(string toUser, string sdp) { var me = Context.User?.Identity?.Name?.Trim().ToLowerInvariant(); var target = (toUser ?? "").Trim().ToLowerInvariant(); Console.WriteLine($"[CALL] Offer from={me} to={target} sdpLen={(sdp ?? "").Length} conn={Context.ConnectionId}"); if (!string.IsNullOrWhiteSpace(me) && !string.IsNullOrWhiteSpace(target)) await Clients.User(target).SendAsync("CallOffer", me, sdp); }
    public async Task SendCallAnswer(string toUser, string sdp) { var me = Context.User?.Identity?.Name?.Trim().ToLowerInvariant(); var target = (toUser ?? "").Trim().ToLowerInvariant(); Console.WriteLine($"[CALL] Answer from={me} to={target} sdpLen={(sdp ?? "").Length} conn={Context.ConnectionId}"); if (!string.IsNullOrWhiteSpace(me) && !string.IsNullOrWhiteSpace(target)) await Clients.User(target).SendAsync("CallAnswer", me, sdp); }
    public async Task SendCallIce(string toUser, string candidate) { var me = Context.User?.Identity?.Name?.Trim().ToLowerInvariant(); var target = (toUser ?? "").Trim().ToLowerInvariant(); Console.WriteLine($"[CALL] ICE from={me} to={target} len={(candidate ?? "").Length} conn={Context.ConnectionId}"); if (!string.IsNullOrWhiteSpace(me) && !string.IsNullOrWhiteSpace(target)) await Clients.User(target).SendAsync("CallIce", me, candidate); }
