...
public static class ServerChannels
{
    public static void Register(WebApplication app)
    {
        ...
        app.MapPost("/api/servers/{sid}/channels", async (string sid, HttpContext ctx) =>
        {
            ...
            if (type != "text" && type != "voice")
                return Results.BadRequest(new { message = "Typ måste vara 'text' eller 'voice'." });
    ...
    public static List<object> GetChannels(string serverId)
    {
        ...
        while (r.Read())
            list.Add(new { channelId = r.GetString(0), name = r.GetString(1),
                           type = r.GetString(2), createdAt = r.GetString(3) });
        return list;
    }
