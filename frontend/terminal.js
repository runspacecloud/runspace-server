...
const COMMANDS = {
  ...
  '/profile/settings':{ desc: 'Öppna inställningar',  action: () => nav('/profile/settings') },
  '/profile': { desc: 'Din profil',            action: () => nav('/profile') },
...
async function executeCommand(raw) {
  ...
  if (lower.startsWith('/profile ')) {
    ...
    setTimeout(() => window.location.href = `/profile?u=${encodeURIComponent(username)}`, 400);
    return;
  }
