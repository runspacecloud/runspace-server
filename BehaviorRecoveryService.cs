...
public class BehaviorRecoveryService : BackgroundService
{
    ...
    private void RunRecovery()
    {
        ...
        cmd.CommandText = @"
            SELECT a.Id, a.Username, a.trust_behavior, a.TrustLevel
            FROM AuthUsers a
            WHERE a.trust_behavior < 98.0
              AND a.IsSuspended = 0
              AND NOT EXISTS (
                  SELECT 1 FROM TrustPenalties p
                  WHERE p.UserId = a.Id
                    AND p.CreatedAt > datetime('now', '-1 hour')
              )
            LIMIT 500";

        var toRecover = new System.Collections.Generic.List<(int id, string username, double behavior, string level)>();
        using var r = cmd.ExecuteReader();
        while (r.Read())
            toRecover.Add((r.GetInt32(0), r.GetString(1), r.GetDouble(2), r.IsDBNull(3) ? "medium" : r.GetString(3)));
        r.Close();

        int count = 0;
        foreach (var (id, username, behavior, level) in toRecover)
        {
            var newBehavior = Math.Min(100.0, behavior + 1.5);
            if (newBehavior - behavior < 0.1) continue;

            using var upd = db.CreateCommand();
            upd.CommandText = "UPDATE AuthUsers SET trust_behavior=$b WHERE Id=$id";
            upd.Parameters.AddWithValue("$b", newBehavior);
            upd.Parameters.AddWithValue("$id", id);
            upd.ExecuteNonQuery();

            var oldDims = new TrustDimensions { Identity = 50, Behavior = behavior, Device = 70 };
            var newDims = new TrustDimensions { Identity = 50, Behavior = newBehavior, Device = 70 };
            ...
            if (oldLevel != newLevel)
            {
                ...
                hist.CommandText = @"INSERT INTO TrustHistory2 (UserId, Timestamp, ReasonCode, Dimension, OldScore, NewScore, Delta)
                ...
                evt.CommandText = @"INSERT INTO SecurityEventLog (UserId, Timestamp, EventType, FromState, ToState, Details)
